class Game {
    constructor(images, sounds){
        this.images = images
        this.sounds = sounds
        this.samCharImages = loadSamImages(this.images.samCharImg, 8)
        this.samCoinImages = loadSamImages(this.images.samCoinsImg, 2)
        this.samEnemyImages = loadSamImages(this.images.samEnemyImg, 3)
        this.numCoins = gameSettings.numCoins
        this.numGoombas = gameSettings.numGoombas
        this.started = false
        this.over = false
        this.hero = new Mario(this.samCharImages, this.sounds.jumpSound, {x: gameSettings.heroStartX, y: gameSettings.heroStartY}, gameSettings.heroSize)
        this.coins = Array.from({ length: gameSettings.numCoins}, (el, i) => {
            return new Coin(this.samCoinImages, this.sounds.coinSound, { x: gameSettings.coinSize * i, y: gameSettings.coinLevel }, gameSettings.coinSize)
        })
        this.goombas = null
    ...
    init = () => {
        if(!this.started){
            ...
            this.goombas = Array.from({length: this.numGoombas}, (el, i) => {
                return new Goomba(this.samEnemyImages, {x: gameSettings.goombaFirstX + (gameSettings.goombaMinSpace * (gameSettings.goombaRandomSpaceMult * Math.random()) * i), y: gameSettings.goombaStartY}, gameSettings.goombaSize)
            })
            this.started = true
            this.over= false
            this.died = false
